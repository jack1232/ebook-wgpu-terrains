...
impl State {
    async fn new(window: &Window, sample_count: u32, width: u32, height: u32) -> Self {
        ...
        let shader = init
            .device
            .create_shader_module(wgpu::include_wgsl!("../ch01/shader_unlit.wgsl"));
    ...
    fn input(&mut self, event: &WindowEvent) -> bool {
        match event {
            WindowEvent::KeyboardInput {
                ...
                VirtualKeyCode::LAlt => {
                    self.terrain.level_of_detail = (self.terrain.level_of_detail + 1) % 9;
                    self.update_buffers = true;
...
fn main() {
    ...
    let window = winit::window::WindowBuilder::new()
        .build(&event_loop)
        .unwrap();
    window.set_title(&*format!("{}", "terrain_water_level"));
